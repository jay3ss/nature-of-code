...
int main()
{
    ...
    sf::RectangleShape magnitude(sf::Vector2f(0, 0));
    ...
    while(window.isOpen())
    {
        ...
        // Set the size of the rectangle (x-direction) based on the magnitude
        // of the mouse vector
        magnitude.setSize(sf::Vector2f(mouse.mag(), 20));

        window.clear();
        window.draw(magnitude);
        window.draw(line);
        window.display();
    }
